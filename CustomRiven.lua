...
OnTick(function ()
	...
	if Mix:Mode() == "Harass" then
		...
		if RivenMenu.Harass.HW:Value() and Ready(_W) and ValidTarget(target, GetCastRange(myHero, _W)) then
				CastSpell(_W)
		end
	...
	if Mix:Mode() == "LaneClear" then
		...
		if RivenMenu.LaneClear.LCW:Value() and Ready(_W) and MinionsAround(myHero, GetCastRange(myHero, _W), MINION_ENEMY) > 1 then
			CastSpell(_W)
		end
	...
	if Mix:Mode() == "LastHit" then
		...
		for _,closeminion in pairs(minionManager.objects) do
			if RivenMenu.LastHit.LHW:Value() and Ready(_W) and ValidTarget(closeminion, GetCastRange(myHero, _W)) then
				if WDmg(closeminion) >= GetCurrentHP(closeminion) then
					CastSpell(_W)
				end
			end
			
			if RivenMenu.LastHit.LHQ:Value() and Ready(_Q) and ValidTarget(closeminion, GetCastRange(myHero, _Q)) then
	...
	for _, enemy in pairs(GetEnemyHeroes()) do
		if RivenMenu.KillSteal.KSW:Value() and Ready(_W) and ValidTarget(enemy, GetCastRange(myHero, _W)) then
			if GetCurrentHP(enemy) + GetDmgShield(enemy) + GetHPRegen(enemy) < WDmg(enemy) then
				CastSpell(_W)
			end
		end		
		
		if RivenMenu.KillSteal.KSQ:Value() and Ready(_Q) and ValidTarget(enemy, GetCastRange(myHero, _Q)) then
			if GetCurrentHP(enemy) + GetDmgShield(enemy) + GetHPRegen(enemy) < QDmg(enemy) then
				CastSkillShot(_Q, enemy)
			end
		end

		if RivenMenu.KillSteal.KSR:Value() and UltOn and Ready(_R) and ValidTarget(enemy, 1100) and WindWall == nil or GetDistance(myHero, WindWall) > 1100 then 
	...
	if UltOn and Ready(_R) then
		for _, camp in pairs(minionManager.objects) do
			...
			if RivenMenu.JCS.JCSB:Value() and GetTeam(camp) == 300 and IsObjectAlive(camp) and GetObjectName(camp) == "SRU_Baron" and ValidTarget(camp, 1100) then
				if GetCurrentHP(camp) < RDragDmg then
					CastSkillShot(_R, camp)
				end
			end
			
			if RivenMenu.JCS.JCSD:Value() and GetTeam(camp) == 300 and IsObjectAlive(camp) and GetObjectName(camp):lower():find("sru_dragon") and ValidTarget(camp, 1100) then
				if GetCurrentHP(camp) < RDragDmg then
					CastSkillShot(_R, camp)
				end
			end
