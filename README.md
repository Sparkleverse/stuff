...
do -- object managers
	_G.heroes = {}
	OnObjectLoad(function(o) if o.type == myHero.type then heroes[1+#heroes] = o end end)
	...
	function GetObjects(_,__,___)
		...
		local function cpairs(_)
			local function IsOk(m,_)if m and m.valid then for k,v in pairs(_)do if(k=="distance")then if m[k]>v then return false end elseif(type(v)=="function"and not v(m)) or m[k] or m[k]~=v then return false end end return true else return false end end
...
do -- misc load
	...
	AutoUpdater(iVersion, true, "gos-scripts.com", "/get.php?version=inspired", "/inspired.lua", COMMON_PATH.."Inspired.lua", function()print("Updated Inspired.lua")end, function()end, function()end, function()end)
	_G.InspiredLoaded = true
end
local hasDac = FileExist(SCRIPT_PATH.."Deftsu's Auto Carry") or FileExist(COMMON_PATH.."Deftsu's Auto Carry")
function LoadOrb(i)
	if i == 1 then
		LoadIOW()
	elseif i == 2 and LoadDAC then
